...
function save_man(){
	...
	if($id){
		...
		if($giamdinh = upload_image("file1", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_baiviet,$file_name1)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_baiviet.$row['giamdinh']);
			}
		}
		...
		$data['chuduan'] = $_POST['chuduan'];
		$data['hangmuc'] = $_POST['hangmuc'];
		$data['sokyhieu'] = $_POST['sokyhieu'];
		$data['ngaybanhanh'] = $_POST['ngaybanhanh'];
		$data['bg_content'] = $_POST['bg_content'];
		$data['masp'] = $_POST['masp'];

		$data['id_trangthai'] = implode(',',$_POST['tt']);

		$data['dinhluong'] = $_POST['dinhluong'];
		$data['id_image'] = $_POST['id_image'];
		$data['id_color'] = $_POST['id_color'];
		$data['id_clarity'] = $_POST['id_clarity'];
...
function delete_man(){
	...
	if(isset($_GET['id'])){
		...
		$d->reset();
		$sql = "select id,photo,thumb,giamdinh from #_baiviet where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
...
function save_list(){
	...
	$file_quangcao1=images_name($_FILES['quangcao1']['name']);
	...
	if($id){
		...
		if($photo = upload_image("quangcao", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_hinhanh,$file_quangcao)){
			$data['quangcao'] = $photo;
		...
		if($photo = upload_image("quangcao1", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_hinhanh,$file_quangcao1)){
			$data['quangcao1'] = $photo;
			$data['quangcao1_thumb'] = create_thumb($data['quangcao1'], $config_cap1['img-qc1-width']*$config_cap1['img-qc1-ratio'],$config_cap1['img-qc1-height']*$config_cap1['img-qc1-ratio'], _upload_hinhanh,$file_quangcao1,1);
			$d->setTable('baiviet_list');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_hinhanh.$row['quangcao1']);
				delete_file(_upload_hinhanh.$row['quangcao1_thumb']);
			}
		}
		foreach ($config['lang'] as $k => $v){
			$data['ten_'.$k] = $_POST['ten_'.$k];
		...
		$data['links1'] = $_POST['links1'];
		$data['bg_title'] = $_POST['bg_title'];
		...
		if($photo = upload_image("quangcao", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_hinhanh,$file_quangcao)){
			$data['quangcao'] = $photo;
			$data['quangcao_thumb'] = create_thumb($data['quangcao'], $config_cap1['img-qc-qc']*$config_cap1['img-qc-ratio'],$config_cap1['img-qc-height']*$config_cap1['img-qc-ratio'], _upload_hinhanh,$file_quangcao,1);
		}
		if($photo = upload_image("quangcao1", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_hinhanh,$file_quangcao1)){
			$data['quangcao1'] = $photo;
			$data['quangcao1_thumb'] = create_thumb($data['quangcao1'], $config_cap1['img-qc1-width']*$config_cap1['img-qc1-ratio'],$config_cap1['img-qc1-height']*$config_cap1['img-qc1-ratio'], _upload_hinhanh,$file_quangcao1,1);
