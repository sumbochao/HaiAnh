...
<div class="wrapper">
	<div class="control_frm">
	    <div class="bc">
	        <ul id="breadcrumbs" class="breadcrumbs">
	        	<li><a href="index.php?com=baiviet&act=add_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span><?=$config_cap2['title']?></span></a></li>
	            <li class="current"><a href="#" onclick="return false;"><?=($_GET['act']=='edit_cat') ? 'Sửa':'Thêm'?></a></li>
	        </ul>
	        <div class="clear"></div>
	    </div>
	</div>
	...
	<form name="supplier" id="validate" class="form" action="index.php?com=baiviet&act=save_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
		<div class="mtop0">
			...
			<div class="<?=($config_cap2['full']==true) ? 'oneOne':'colLeft' ?>">
				<div class="widget mtop0">
			...
			<div class="<?=($config_cap2['full']==true) ? 'oneOne':'colRight' ?>">
				<div class="widget mtop0">
					<div class="title"><img src="./images/icons/dark/record.png" alt="" class="titleIcon" />
						<h6>Danh mục</h6>
					</div>
					...
					<div class="formRow">
						...
						<div class="formRight">
							...
							<span style="display: inline-block; line-height: 30px;color:#CCC; padding-left: 10px;">
						 	Width: <?=$config_cap2['img-width']*$config_cap2['img-ratio']?>px - <?=$config_cap2['img-height']*$config_cap2['img-ratio']?>px
						 </span>
						</div>
						<div class="clear"></div>
					</div>

			        <?php if($_GET['act']=='edit_cat' && $item['thumb']!=''){?>
					<div class="formRow">
		...
		<div class="formRow fixedBottom">
			<div class="formRight">
	        	...
	        	<a href="index.php?com=baiviet&act=man_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="button tipS redB" original-title="Thoát">Thoát</a>
			</div>
			<div class="clear"></div>
		</div>
	</form>
