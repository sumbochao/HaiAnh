...
function getLink($str,$index=false,$html=false){
	if($index==true){
		return '<a href="" title="'.$str.'">'.$str.'</a>';
	}else{
		if($html==true){ $h = '.html'; }
		return '<a href="'.changeTitle($str).$h.'" title="'.$str.'">'.$str.'</a>';
	}
...
function encrypt_password($str,$salt){
	global $config;
	return md5($config['bimat'].$str.$salt);}
...
function getAuthor($pid){
	global $d, $row,$lang;
	$sql = "select username from #_user where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row['username'];
}
function get_thuonghieu($pid){
	global $d, $row,$lang;
	$sql = "select ten_$lang from #_baiviet_list where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row['ten_'.$lang];
}
function getListBV($pid){
	global $d,$lang;
	$sql = "select ten_$lang as ten, tenkhongdau from #_baiviet_list where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row;
}
...
function upload_video($file, $extension, $folder, $newname=''){
